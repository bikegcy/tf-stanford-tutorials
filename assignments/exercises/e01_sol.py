...
# out = tf.case(tf.less(x,y): tf.add(x, y), tf.greater(x, y): tf.subtract(x, y))
...
out = tf.equal(tf.shape(x), tf.shape(y))
print('x :', x, ' y:', y)
print('out:', out)
...
sub = x - y
condition = tf.less(tf.reduce_mean(sub), 0)
neg_res = tf.reduce_mean(tf.square(sub))
pos_res = tf.reduce_sum(tf.abs(sub))
out = tf.cond(condition, lambda: neg_res, lambda: pos_res)
print('out: ', out)
